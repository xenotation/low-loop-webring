...
function createLayer(e, t, i, o, s, a) {
    cE(e);
    var r = gE(e);
    return sC(r, 0, o, s, 0), sE(r), sX(r, t), (r.myx = t), sY(r, i), (r.myy = i), wH(r, a), sZ(r, 1e3), r;
}
function Neko(x, y, active, imagedirectory) {
    ...
    (this.layer = createLayer(strLayer, x, y, 32, 32, strContent)),
        ...
        window.stepsize && window.stepsize > 0 ? (this.stepsize = window.stepsize) : (this.stepsize = 16),
        active || (active = !1),
        (this.active = active),
...
function NekoThink() {
    this.active ? this.TargetMouse() : this.TargetHome(), this.CalculateDistance(), this.FindDirection(), this.UpdateImage(), "chasing" == this.behavior && this.MoveAStep();
...
// MODFICTION
window.startNekoX = Math.floor(window.innerWidth / 2 - 16); // Offset to center (16px is half the width of the Neko image)
window.startNekoY = Math.floor(window.innerHeight / 2 - 16); // Offset to center (16px is half the height of the Neko image)
...
function boxSetBoard(e, t, i) {
    ...
    var a = aNekos[e];
    return (a.boardX == o && a.boardY == s) || (-1 != a.boardX && checkerboard[a.boardX][a.boardY]--, checkerboard[o][s]++, (a.boardX = o), (a.boardY = s)), 0 == checkerboard[o][s] ? 0 : checkerboard[o][s] - 1;
}
...
var checkerboardEccentricity = 10,
    ...
    nDelayVariance = 20,
    nFirstRealFrame = 5,
    sNekoMessage = "Click Neko and he'll chase your mouse!    Double-click to visit Neko's home (https://webneko.net)",
